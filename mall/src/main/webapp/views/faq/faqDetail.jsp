...
<script type="text/javascript">
	...
	$(function(){
		...
		$(".commit-btn").click(function(){
			...
			var url = project_fullpath +"/feedbackCtrl/updateFaqInfo";
			$.post(url,params,function(result){
				if(result == 1){
					layer.msg("保存成功!",{icon: 0,shift: 4,offset: '100px'});
				}else{
					layer.msg('保存失败!', {offset: '100px',shift: 6});
				}
			},"json");
		})
...
<body style="background-color:#fff;">
	<div style="padding-top:10px;margin:0">
		<table id="detailInfo">
			<tr class="alone">
				<td><span>标题:</span></td>
				<td colspan="3"><input type="text" id="title" value="${detailInfo.subtitle}"></td>
			</tr>
			<tr>
				<td><span>主题颜色:</span></td>
				<td><ul class="theme-color no-padding">
					<c:forEach var="info" items="${color}">
						<c:if test="${info.key == detailInfo.colorClass}">
							<li class="${info.value} active transition" 
							data-value="${info.key}"
							></li>
						</c:if>
						<c:if test="${info.key != detailInfo.colorClass}">
				...
				<td><span>最后修改时间:</span></td>
				<td><span id="lastModifyTime">${detailInfo.lastModifyTime}</span></td>
			</tr>
			<tr class="alone">
				<td style="vertical-align:middle;"><span>是否默认展开:</span></td>
				<td colspan="3">
					<ul class="ul-inline">
						<li><input name="isOpen" <c:if test="${detailInfo.isOpen != 1}">checked</c:if> type="radio" value="0" class="flat-grey" id="isOpen-n"> <label class="simple-label" for="isOpen-n">否</label></li>
						<li><input name="isOpen" <c:if test="${detailInfo.isOpen == 1}">checked</c:if> type="radio" value="1" class="flat-grey" id="isOpen-y"> <label class="simple-label" for="isOpen-y">是</label></li>
					</ul>
				</td>
			</tr>
			<tr class="alone">
			...
			<tr class="alone">
				...
				<td colspan="3">
					<ul class="ul-inline">
						<c:forEach var="info" items="${typeInfo}">
						<c:if test="${info.id == detailInfo.fid}">
			...
			<tr class="alone">
				<td><span>内容:</span></td>
				<td colspan="3">
					<script type="text/plain" id="uedit" style="width:100%;height:230px;">${detailInfo.content}</script>
				</td>
			</tr>
			<tr class="alone">
				<td></td>
				<td colspan="3"><input type="hidden" id="fid" value="${detailInfo.fid}">
