...
<script type="text/javascript">
	...
	$(function(){
		...
		$(".commit-btn").click(function(){
			...
			var url = project_fullpath +"/feedbackCtrl/updateVersionInfo";
			$.post(url,params,function(result){
				if(result == 1){
					layer.msg("修改成功!",{icon: 0,shift: 4,offset: '100px'});
				}else{
					layer.msg('修改失败!', {offset: '100px',shift: 6});
				}
			},"json");
		})
...
<body style="background-color:#fff;">
	<div style="padding-top:10px;margin:0">
		<table id="detailInfo">
			<tr class="alone">
				<td><span>标题:</span></td>
				<td colspan="3"><input type="text" class="form-contorl" id="title" value="${detailInfo.title}" ></td>
			</tr>
			<tr class="alone">
				<td style="vertical-align:middle;"><span>版本号:</span></td>
				<td colspan="3">
					<input type="text" class="form-contorl" id="versionNum" value="${detailInfo.versionNum}" >
				</td>
			</tr>
			<tr class="alone">
				<td style="vertical-align:middle;"><span>更新时间:</span></td>
				<td colspan="3">
					<input type="text" id="time" class="laydate-icon" value="${detailInfo.time}">
				</td>
			</tr>
			<tr class="alone">
				...
				<td colspan="3">
					<ul class="ul-inline">
						<li><input name="triggered" <c:if test="${detailInfo.triggered != 1}">checked</c:if> type="radio" value="0" class="flat-grey" id="triggered-n"> <label class="simple-label" for="triggered-n">否</label></li>
						<li><input name="triggered" <c:if test="${detailInfo.triggered == 1}">checked</c:if> type="radio" value="1" class="flat-grey" id="triggered-y"> <label class="simple-label" for="triggered-y">是</label></li>
					</ul>
				</td>
			</tr>
			<tr class="alone">
				...
				<td colspan="3">
					<ul class="ul-inline">
						<li><input name="isPublish" <c:if test="${detailInfo.isPublish != 1}">checked</c:if> type="radio" value="0" class="flat-grey" id="isPublish-n"> <label class="simple-label" for="isPublish-n">否</label></li>
						<li><input name="isPublish" <c:if test="${detailInfo.isPublish == 1}">checked</c:if> type="radio" value="1" class="flat-grey" id="isPublish-y"> <label class="simple-label" for="isPublish-y">是</label></li>
					</ul>
				</td>
			</tr>
			<tr class="alone">
				...
				<td colspan="3">
					<textarea id="briefContents" class="form-control" style="resize: none;" rows="3" cols="80%">${detailInfo.briefContents}</textarea>
				</td>
			</tr>
			<tr class="alone">
				<td><span>更新详细内容:</span></td>
				<td colspan="3">
					<textarea id="detailContents" class="form-control" style="resize: none;" rows="5" cols="80%">${detailInfo.detailContents}</textarea>
				</td>
			</tr>
			<tr class="alone">
				<td></td>
				<td colspan="3">
					<button type="submit" class="commit-btn">修改</button>
				</td>
			</tr>
		</table>
